...
def auto_ingest_pipeline(mode="BOTH"):
    """
    This is the function used for grabbing the csv files out of the watch directory.
    That directory is where vision builder exports its named csv files.
    This function relies on the files having LHS/RHS as the first part of its file name.
    Also checks the time of creation of the file so that only the most recent files are pulled in for calculations.
    """
...
def status_network():
    """
    This function handles the queue, and payloads that the threads and other functions kick out
    Essentially an event handler for the GUI display and automatic triggering of functions
    """
    global g_master_df
    try:
        while True:
...
def open_settings_window():
    ...
    tk.Button(maint_lf, text="Clear Dashboard Logs", command=clear_all_data, width=54, bg="#f8d7da",
              fg="#842029").pack(pady=2)

    tk.Button(settings_win, text="Open Live Comms List", command=open_io_list_window, width=22, bg="#0c447c",
    ...
    tk.Button(clear_row, text="Clear Manual Flags", command=manual_vb_clear_flags, width=70, bg="#f8d7da",
              fg="#842029").pack()
    ...
    while g_system_running:
        ...
        if g_connection_vb is None:
            try:
                ...
                l_connection_vb.settimeout(10.0)
        ...
        try:
            ...
            # went low - send one more packet so VB sees the trigger go to 0 too.
            # The packet-build below already reads live g_plc_rx_* values, so it naturally sends a 0 for
            # whichever bit just dropped.
            # Kept separate from the rising-edge booleans because only a genuine new request should
            # drive the receive-side pass/fail mapping.
            ...
            if do_camera_send or do_barcode_send:
                ...
                # this guards against a stale LH or RH "required" bit sneaking through to VB when
                ...
                outbound_packet = struct.pack("<BBH", tx_b0, 0, int(l_vb_position)) #swapped ! to < for BE to little endian
                g_connection_vb.sendall(outbound_packet)
                g_vb_send_done = True #sets the flag to High to indicate that the packet was sucessfully built
                just_sent = True      #sets the flag to High to be used for an IF check for the receive block of thread_vb
...
#======================================= Receive ==============================================
            ...
            if just_sent:  #checks the status of the tag before entering the blocking recv() portion of thread_vb
                ...
                # blocking on a receive with nothing coming. Short sleep while idle,
            ...
            while g_system_running and session_active:
                try:
                    ...
                    byte0, _, byte2, _, _, robot_pos, _, master_csv_bytes, recipe_selection = struct.unpack(
                        "!BBBBHH50s20sH", data[:80]) #unpacks and decodes the structure sent from PLC
                    ...
                    if bool(byte0 & (1 << 6)):  #checks if the Capture results bit from the plc rx flags is true/high
                    ...
                    # and plc_req_rh_barcode are not defined in this file, and thread_vb() does
                    # not read .pending_trigger anymore, just left it here incase ever wnat to use it again but probably not
...
# Start threads
#Note that we are using daemon threads so that they can be run in the background whilst everything is working
thread_vb.pending_trigger = None  #not used any more, hold over from previous version of code


threading.Thread(target=thread_plc, daemon=True).start()  #starts the thread that handles comms between the PLC and Python

threading.Thread(target=thread_vb, daemon=True).start()  #starts the thread that handles comms between Python and Vision Builder

threading.Thread(target=thread_heartbeat, daemon=True).start()  #independent thread that just handles the heartbeat between Python and PLC
