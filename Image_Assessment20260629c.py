...
def thread_vb():
    ...
    while g_system_running:
        ...
        try:
            ...
            # Camera: clear both the private gate AND the PLC-facing fields here, for a direct trigger_camera
            # cycle that completes on its own. A SEPARATE clear also exists in thread_plc, tied to the
            # Capture Results falling edge, for when execute_assessment() sets these same fields for the
            # overall tolerance result instead - trigger_camera and Capture Results are independent
            # operations that happen to share this TX field, so both need their own clear path. Safe to have
            # both active: clearing is idempotent, and gating itself no longer reads these fields at all.
            ...
            # Barcode: nothing else in the codebase writes to g_plc_tx_barcode_pass/fail, so there's no
            # conflict here like there is for camera - clear both the private gate AND the PLC-facing
            # fields together, same as before the camera/execute_assessment conflict was found.
