...
def auto_ingest_pipeline(mode="BOTH"):
    ...
    if mode == "LHS" or mode == "BOTH":
        ...
        # Scan newest-first: for each position slot, keep the first (newest) file that passes the
        # 77-point integrity check. Earlier incomplete retry files are skipped transparently.
        for path, filename, _ in lhs_candidates:
            if len(g_lh_positions_db) == 5: break  # all 5 slots filled with valid data
            detected_pos = None
            for i in range(1, 6):
