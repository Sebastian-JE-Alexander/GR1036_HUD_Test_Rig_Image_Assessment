...
def open_settings_window():
    ...
    settings_win.title("Settings Panel");
    ...
    tk.Button(sync_lf, text="Synchronize Full Dataset (10 Files)", command=lambda: auto_ingest_pipeline("BOTH"),
    ...
    tk.Button(clear_row, text="Clear Manual RX Bit Flags", command=manual_vb_clear_flags, width=70, bg="#f8d7da",
              fg="#842029").pack()
...
def thread_vb():
    ...
    # One-shot: track whether we've already dealt with the current high-state of each
    # PLC trigger. turn off only once the PLC drops the bit back to False, so a held-high
    # bit fires exactly once instead of resending on every loop pass. causing flooding of data to PLC
    l_camera_trigger_armed = False
    l_barcode_trigger_armed = False
    ...
    while g_system_running:
        ...
        try:
            #============================= Send ============================================================
            do_camera_send = (g_plc_rx_trigger_camera is True) and (not l_camera_trigger_armed)   #updates local camera trigger value only when PLC puts it High
            do_barcode_send = (g_plc_rx_capture_barcode is True) and (not l_barcode_trigger_armed) #updates local barcode trigger value only when PLC puts it High
            ...
            if do_camera_send or do_barcode_send:
                ...
                outbound_packet = struct.pack("<BBH", tx_b0, 0, int(l_vb_position)) #swapped ! to < for big endian to little endian
                g_connection_vb.sendall(outbound_packet)
                g_vb_send_done = True

                if do_camera_send: l_camera_trigger_armed = True
                if do_barcode_send: l_barcode_trigger_armed = True

            # Turns off once the PLC actually drops the bit, so its next High bit is treated as a fresh trigger
