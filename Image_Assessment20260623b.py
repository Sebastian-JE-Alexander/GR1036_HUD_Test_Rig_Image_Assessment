    ...
    while g_system_running:
        ...
        if g_connection_vb is None:
            try:
                ...
                l_connection_vb.settimeout(15.0) #set higher socket timeout
        ...
        try:
            ...
            # Gate on the PLC TX pass/fail fields rather than a latch: a trigger is sent only
            # while it's high AND we haven't already recorded a pass or fail for it. Those fields get set
            # from VB's reply, and cleared once the PLC drops its trigger bit.

            #================ NEW Andrew vision builder interlock using falling/negative edge ========================
            # Falling edge: if pass/fail is still set when the PLC bit drops, that's the moment it just
            # went low - send one more packet so VB sees the trigger go to 0/False too, not just our own state in python.

            camera_rising_send = (g_plc_rx_trigger_camera is True) and (g_plc_tx_camera_pass is not True) and (
                        g_plc_tx_camera_fail is not True)  #normal message send on rising/positive edge
            barcode_rising_send = (g_plc_rx_capture_barcode is True) and (g_plc_tx_barcode_pass is not True) and (
                        g_plc_tx_barcode_fail is not True) #normal message send on rising/positive edge

            camera_falling_edge = (g_plc_rx_trigger_camera is False) and (
                        g_plc_tx_camera_pass is True or g_plc_tx_camera_fail is True) #reaction message on falling/negative edge
            barcode_falling_edge = (g_plc_rx_capture_barcode is False) and (
                        g_plc_tx_barcode_pass is True or g_plc_tx_barcode_fail is True) #reaction message on falling/negative edge

            do_camera_send = camera_rising_send or camera_falling_edge #works on whether it was a positive or negative edge message
            ...
            if do_camera_send or do_barcode_send:
                ...
                #Passes the local variables into the byte structure before sending the packet
                tx_b0 = 0
                ...
                #Builds the packet using the structure of byte, byte, integer(two bytes)
                outbound_packet = struct.pack("<BBH", tx_b0, 0, int(l_vb_position)) #swapped from ! to < for big endian to little endian
                g_connection_vb.sendall(outbound_packet)
                g_vb_send_done = True

            # Clear pass/fail bits once the PLC drops the bit, so the next Rising/Positive edge starts a fresh cycle
...
#======================================= Receive ==============================================
            ...
            # genuine new request (Rising/Positive edge), not the Falling/Negative edge "trigger now off" notification above.
            ...
            time.sleep(1.0)  #delay before retrying to establish connection to vision builder
            ...
            def plc_cyclic_sender(sock):
                ...
                while g_system_running and session_active:
                    try:
                        ...
                        if (timer_current - timer_prev).total_seconds() > plc_send_rate: #checks the time to see if we are matching the send rate
                            timer_prev = timer_current #if send rate matched, then update previosu timer to new time of send
...
#======================================== Receive =====================================================
            while g_system_running and session_active:
                try:
                    ...
                    #checks if the capture results bit is High, then checks the recipe selection byte to know what we doing
                    if bool(byte0 & (1 << 6)):
...
# For creating the 5 position label buttons on the GUI for 
