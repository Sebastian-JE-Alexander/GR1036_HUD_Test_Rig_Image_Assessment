...
# Drift chart buttons — one per position per variant, sitting directly below the status buttons
tk.Label(global_frame, text="LHS Drift Charts: ", font=("Arial", 9, "bold"), fg="#555").grid(
    row=2, column=0, padx=5, pady=(0, 4), sticky="w")
for i in range(1, 6):
    tk.Button(global_frame, text=f"📈 Pos {i}", width=10, bg="#e8f4fd", fg="#0c447c", font=("Arial", 8),
              command=lambda pos=i: show_drift_chart("LHS", pos)).grid(row=2, column=i, padx=8, pady=(0, 4))
...
# Overall pass/fail label sits to the right of the position buttons, spanning all four rows
