"""
GR1036 HUD Test Rig
Image Assessment GUI & PLC / NI Vision Information Broker

This script builds a GUI for displaying pass/fail status indicators and individual calculated
parameters for each robot position.

Also handles TCP communications between the PLC inside the Control Panel and NI Vision Builder on the local PC.
These comms are handled by means of a data structure using a combination of boolean flags to indicate triggers
and byte combinations for messages.
...
# Network Configuration parameters for PLC and Vision Builder
PLC_PORT = 9005
VBAI_IP = "127.0.0.1"
VBAI_PORT = 9006
plc_send_rate = 0.200
...
def change_watch_directory():
    """
    Function to allows us to call the directory and select which folder on the PLC that we want to direct the script to look at
    when going to find where Vision Builder is exporting its CSV files to.
    """
...
def auto_ingest_pipeline(mode="BOTH"):
    ...
    if mode == "BOTH":
        test_label.config(text=f"Data: {loaded_lh} LHS / {loaded_rh} RHS (10 Files)", fg="green",
                          font=("Arial", 9, "bold"))
    elif mode == "LHS":
        test_label.config(text=f"Data: {loaded_lh} LHS Only", fg="#0dcaf0", font=("Arial", 9, "bold"))
    elif mode == "RHS":
        test_label.config(text=f"Data: {loaded_rh} RHS Only", fg="#ffc107", font=("Arial", 9, "bold"))

    check_run_conditions()
    if g_master_df is not None:
        log_message(f"[PLC] Recipe '{mode}' load complete ({loaded_lh} LHS / {loaded_rh} RHS) - running assessment")
        execute_assessment()
    else:
        log_message(f"[PLC] Recipe '{mode}' load complete ({loaded_lh} LHS / {loaded_rh} RHS) - "
...
def save_assessment_report():
    ...
    try:
        with open(file_path, mode='w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow(["GR1036 HUD Test Rig - Assessment Report"])
                ...
                for key, data in metrics.items():
                    ...
                    writer.writerow(
                        ["RHS", f"Position {pos}", label, master_txt, test_txt, tol_inputs[key].get(), variance_txt,
                         status_txt.strip()])
        messagebox.showinfo("Save Report", "Assessment report successfully saved.")
    except Exception as e:
        messagebox.showerror("Report Export Failure", str(e))
...
def process_variant_database(source_db, results_db, m_res, overview_buttons):
    """
    Function that handles running the calculations for the loaded data and tracking each parameter individual pass/fail when compared
    against the loaded tolerances
    """
    any_fail = False
    for i in range(1, 6):
...
def execute_assessment():
    """
    Top Level function that calls in everything from tags to functions to be called when we want to run the image assessment on
    the data we've got back from vision builder. This is also the function that handles the overall pass/fail shown on the GUI
    """
...
# sends the structure on its next pass using its own, unmodified send logic.

def log_message(text):
    """
    Allows us to write text messages with timestamps to log window on the ui for any debugging cases
    """
...
def manual_vb_send(camera_trigger, lhs_active, rhs_active, capture_barcode, lh_bc_req, rh_bc_req):
    """
    Only used for debugging purposes, allows us to mimic the triggers sent by the PLC and pass them onto vision builder
    without needing the PLC to have actually sent them, not they must be used carefully as they do mess with what the PLC
    is sending.
    So rather try and use what's on the HMI in manual mode to test triggers, but they can still be used whilst
    ...
    log_message(f"[MANUAL TEST] Cam:{camera_trigger} BC:{capture_barcode} LHS:{lhs_active} "
                f"RHS:{rhs_active} LH_BC:{lh_bc_req} RH_BC:{rh_bc_req} Pos:{test_pos}")  #writes a message to the log window to indicate what was sent using the manual buttons


def manual_vb_clear_flags():
    """
    Very important function to be used in conjunction with the manual send buttons. Since we are mimicing PLC triggers we need to manually clear the trigger flags
    after using the manual send buttons, so that the PLC has clean tags in python to pass info onto and then onto vision builder
    """
    global g_plc_rx_trigger_camera, g_plc_rx_capture_barcode
    global g_plc_rx_lhs_sequence_active, g_plc_rx_rhs_sequence_active
    global g_plc_rx_lh_barcode_req, g_plc_rx_rh_barcode_req
...
def open_io_list_window():
    """
    Used for creating an IO list window in the settings menu that works similarlly to the IO list on the rigs HMI.
    Main difference is that this one focuses on seeing what the data being sent between the PLC, Python and Vision Builder.
    Helpful for debugging comms between the three systems.
    """
...
#================================================== Vision Builder to Python List (RX) ======================================
...
#========================================= Python to PLC List (TX) =====================================================
    ...
    def refresh():
        """
        Function used for refreshing the tags shown in the io list in real time as they are being written to by the PLC and Python.
        """
...
def open_settings_window():
    ...
    config_lf = tk.LabelFrame(settings_win, text=" Config Controls ", padx=10, pady=8);
    ...
    sync_lf = tk.LabelFrame(settings_win, text=" Target Data Overrides ", padx=10, pady=8);
    ...
    maint_lf = tk.LabelFrame(settings_win, text=" Data Controls ", padx=10, pady=8);
    maint_lf.pack(fill="x", padx=15, pady=5)
    tk.Button(maint_lf, text="Clear Dashboard Data Logs", command=clear_all_data, width=54, bg="#f8d7da",
              fg="#842029").pack(pady=2)

    tk.Button(settings_win, text="Open Live IO Comms List", command=open_io_list_window, width=22, bg="#0c447c",
              fg="white").pack(pady=(8, 2))

    vb_test_lf = tk.LabelFrame(settings_win, text=" Manual VBAI Test Panel (Debugging Use Only) ", padx=10, pady=8,
                               fg="#842029");
    ...
    tk.Label(vb_test_lf, text="Sends the same structure thread_vb() already sends, via the PLC RX flags it reads. "
                              , font=("Arial", 8), fg="#6c757d", wraplength=420,
...
def thread_vb():
    ...
    #other global tags that get used
    ...
    while g_system_running:
        ...
        try:
            ...
            if do_camera_send or do_barcode_send:
                ...
                outbound_packet = struct.pack("<BBH", tx_b0, 0, int(l_vb_position)) #swapped ! to < to change from Big Endian to Little Endian.
                g_connection_vb.sendall(outbound_packet)
                g_vb_send_done = True
                just_sent = True  #sets both tags to High/True so that we can use them for evaluating and controlling the gates inside this thread to prevent hanging code.
...
#======================================= Receive ==============================================
            ...
            if just_sent: #checks the Flag before entering into this section as recv() is blocking and can cause python to hang and slow down comms
                ...
                # Otherwise a stale reply to that off-packet could re-set a flag we just cleared.
                ...
                # blocking on a receive with nothing coming. Short sleep avoids hanging the CPU while idle,
...
def thread_heartbeat():
    """
    whilst the thread is running we pulse the heartbeat bit in the data structure sent to the PLC at
    a 1-second interval as requested by the PLC programmer.
    """
...
# ============================ GUI WINDOW LAYOUT CONSTRUCTION =======================================

root = tk.Tk();
root.title("GR1036 HUD Test Rig Dashboard");
root.geometry("1180x700")

#============================ COMPANY LOGOS AND DASHBOARD TITLE ==========================================
...
#================================= POSITION SLOT SELECTION OVERVIEW =========================================================
...
#======================================== ASSESSMENT CALCULATED VARIANCE DISPLAY ======================================
...
#==================================== GUI FOOTER RUNTIME STATUS INDICATORS =================================================
...
# Start threads
thread_vb.pending_trigger = None  #unused thread, holdover from old version of the code


threading.Thread(target=thread_plc, daemon=True).start()  #starts the thread that handles tcp comms between the PLC and Python

threading.Thread(target=thread_vb, daemon=True).start()  #starts the thread that handles tcp comms between Python and Vision Builder

threading.Thread(target=thread_heartbeat, daemon=True).start()  #independent thread that just handles the heartbeat between the PLC and Python
