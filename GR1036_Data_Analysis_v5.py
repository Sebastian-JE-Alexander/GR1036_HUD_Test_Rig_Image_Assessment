...
import numpy as np
import matplotlib
matplotlib.use("Agg")  # must be set before pyplot import when embedding in tkinter
    ...
    try:
        while True:
            ...
            if event_type == "PLC_CONNECTION":
                ...
                # Deliberately does NOT clear g_master_positions_db or the position databases - only the display.
                for key in ui_rows:
...
def show_drift_chart(variant, position_idx):
    ...
    popup.title(f"Drift Analysis — {variant} Position {position_idx}")
    ...
    sc = ax.scatter(x_ghost, y_ghost, c=mag, cmap='viridis', s=marker_size,
                    edgecolor='black', linewidth=0.9, alpha=0.93, zorder=10,
                    label='Ghost points (with magnitude)')
    ax.quiver(x_prim, y_prim, u_dir, v_dir,
    ...
    cbar.set_label('Magnitude of Drift (pixels)', rotation=270, labelpad=18)
    ax.legend(loc='upper right', fontsize=8)
    ax.set_title(f'HUD Drift Analysis — {variant} Position {position_idx}\n'
                 '(Color/Size = Magnitude  •  Arrows = Drift Direction)',
                 fontsize=13, pad=12)
    ax.set_xlabel('X Co-ordinates (pixels)')
    ax.set_ylabel('Y Co-ordinates (pixels)')
    fig.tight_layout()
...
def thread_vb():
    ...
    # Initialise all per-loop-pass state before the while so values don't bleed between passes
    do_camera_send = False
    do_barcode_send = False
    just_sent = False
    L_VBSendIterationComplete = False
    ...
    while g_system_running:
        ...
        try:
            ...
            # previous pass can never trigger a spurious send on the next one.
            # do_camera_send = False
            # do_barcode_send = False
            # just_sent = False
            ...
            # L_VBSendIterationComplete blocks re-sending while the current trigger is still held high
            # after a send has already been completed. Brackets added to make precedence explicit -
            ...
            if (camera_rising_send is True) or (camera_falling_edge is True and L_VBSendIterationComplete is False):
                do_camera_send = True

            if (barcode_rising_send is True) or (barcode_falling_edge is True and L_VBSendIterationComplete is False):
            ...
            if do_camera_send or do_barcode_send:
                ...
                g_vb_send_done = True
                just_sent = True
                L_VBSendIterationComplete = True
                do_camera_send = False
                do_barcode_send = False
            ...
            # Clear pass/fail once the PLC drops the bit and we're not mid-send (just_sent is False
            # means no send happened this pass - the trigger has dropped without a concurrent send).
...
#======================================== Receive =====================================================
            ...
            while g_system_running and session_active:
                try:
                    ...
                    # Only fire the load event when the filename actually changes - avoids log spam and
...
# Row 0: LHS position status buttons
...
# Row 1: LHS drift chart buttons — directly under the LHS status buttons
tk.Label(global_frame, text="LHS Drift Charts: ", font=("Arial", 9, "bold"), fg="#555").grid(
    row=1, column=0, padx=5, pady=(0, 4), sticky="w")
for i in range(1, 6):
    tk.Button(global_frame, text=f"📈 Pos {i}", width=10, bg="#e8f4fd", fg="#0c447c", font=("Arial", 8),
              command=lambda pos=i: show_drift_chart("LHS", pos)).grid(row=1, column=i, padx=8, pady=(0, 4))

# Row 2: RHS position status buttons
tk.Label(global_frame, text="RHS Position Slots: ", font=("Arial", 9, "bold")).grid(row=2, column=0, padx=5, pady=5,
                                                                                    sticky="w")
for i in range(1, 6):
    sf = tk.Frame(global_frame);
    sf.grid(row=2, column=i, padx=8, pady=5)
    tk.Label(sf, text=f"Pos {i}: ", font=("Arial", 9, "bold")).pack(side="left")
    btn = tk.Button(sf, text="IDLE", bg="lightgray", width=8,
...
# Row 3: RHS drift chart buttons — directly under the RHS status buttons
tk.Label(global_frame, text="RHS Drift Charts: ", font=("Arial", 9, "bold"), fg="#555").grid(
    row=3, column=0, padx=5, pady=(0, 4), sticky="w")
for i in range(1, 6):
    tk.Button(global_frame, text=f"📈 Pos {i}", width=10, bg="#fff8e8", fg="#856404", font=("Arial", 8),
              command=lambda pos=i: show_drift_chart("RHS", pos)).grid(row=3, column=i, padx=8, pady=(0, 4))

# Overall pass/fail label spans all four rows on the right
...
overall_status_lbl.grid(row=0, column=6, rowspan=4, padx=(20, 5), pady=5, sticky="nsew")
