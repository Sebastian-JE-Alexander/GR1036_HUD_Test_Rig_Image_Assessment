...
g_master_df = None
g_watch_directory = "C:\\VBAI_Data_Exports"  # Sets the Default PATH for where to check the vision builders exported .csv files
g_master_csv_directory = "C:\\VBAI_Master_Files"  # Folder where master/tolerance CSVs the PLC references by filename 
...
def change_watch_directory():
    ...
    selected_dir = filedialog.askdirectory(title="Select Vision Builder  Data Output Directory")
    if selected_dir:
...
def open_settings_window():
    ...
    tk.Button(sync_lf, text="Synchronize LHS+RHS (10 Files)", command=lambda: auto_ingest_pipeline("BOTH"),
              width=54, bg="#212529", fg="white").grid(row=1, column=0, columnspan=2, padx=5, pady=4)
    maint_lf = tk.LabelFrame(settings_win, text=" Dashboard ", padx=10, pady=8);
    maint_lf.pack(fill="x", padx=15, pady=5)
    tk.Button(maint_lf, text="Clear Dashboard Logs & Arrays", command=clear_all_data, width=54, bg="#f8d7da",
            ...
            while g_system_running and session_active:
                try:
                    ...
                    if g_plc_rx_capture_results is True and not g_capture_results_armed:
                        ...
                        # Recipe selection: 0 = BOTH, 1 = LHS, 2 = RHS (per PLC programmer)
...
try:
    ...
    if os.path.exists(logo1_path):
        logo1_pil = Image.open(logo1_path).resize((155, 55), Image.Resampling.LANCZOS)
...
test_label = tk.Label(summary_frame, text="Test Files Not Loaded Yet", fg="red", font=("Arial", 9, "bold"), bg="#f8f9fa",
                      width=40, anchor="w");
...
for row_idx, (key, label_text) in enumerate(metrics_list, start=2):
    ...
    s_box = tk.Label(matrix_frame, text=" IDLE ", bg="lightgray", font=("Arial", 13, "bold"), borderwidth=1,
...
defaults = {'size': '10.0', 'rotation': '2.0', 'trap_h': '3.0', 'trap_v': '3.0', 'ar': '0.2', 'trans_x': '20.0',
            'trans_y': '20.0', 'smile_h': '3.0', 'smile_v': '3.0', 'ghosting': '3.0'}
