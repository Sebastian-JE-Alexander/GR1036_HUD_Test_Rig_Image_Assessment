...
def thread_vb():
    ...
    # One-shot latches: track whether we've already serviced the current high-state of each
    # PLC trigger bit. Disarmed only once the PLC drops the bit back to False, so a held-high
    # bit fires exactly once instead of resending on every loop pass.
    ...
    while g_system_running:
        ...
        try:
            #============================= Send ============================================================
            do_camera_send = (g_plc_rx_trigger_camera is True) and (not l_camera_trigger_armed)
            do_barcode_send = (g_plc_rx_capture_barcode is True) and (not l_barcode_trigger_armed)
            ...
            # Disarm once the PLC actually drops the bit, so its next rising edge is treated as a fresh trigger
