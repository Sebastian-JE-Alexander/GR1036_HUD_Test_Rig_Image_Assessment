...
def open_io_list_window():
    ...
    io_win.title("Live IO Flags List")
...
def thread_vb():
    ...
    while g_system_running:
        ...
        try:
            ...
            do_camera_send = (g_plc_rx_trigger_camera is True) and (g_plc_tx_camera_pass is not True) and (
                        g_plc_tx_camera_fail is not True)
            do_barcode_send = (g_plc_rx_capture_barcode is True) and (g_plc_tx_barcode_pass is not True) and (
            ...
            if do_camera_send or do_barcode_send:
                ...
                outbound_packet = struct.pack("<BBH", tx_b0, 0, int(l_vb_position)) #swapped ! to < for endian change
...
#======================================= Receive ==============================================
            ...
            # Map VB's result bits onto the PLC TX pass/fail fields - but only for whichever trigger type
            # we actually just sent this cycle. Camera and barcode are separate bracnhes in VB, so a
            # reply for one may carry stale/irrelevant bits for the other; only update what's relevant.
            if do_camera_send:
...
#======================================== Receive =====================================================
            while g_system_running and session_active:
                try:
                    ...
                    if bool(byte0 & (1 << 6)):  #checks IF capture results bit is True then:
                        # Recipe selection: 0 = LHS, 1 = RHS, 2 = BOTH (per PLC spec)
                        if recipe_selection == 0:
                            ingest_mode = "LHS"  #sync only the LHS files (5)
                        elif recipe_selection == 1:
                            ingest_mode = "RHS" #sync only the RHS files (5)
                        else:
                            ingest_mode = "BOTH" #sync both LHS + RHS files (10)
                        # auto_ingest_pipeline() syncs the right files AND runs execute_assessment() itself by being its own function
                        # once done - no separate PLC_CAPTURE_RESULTS push needed here, that end up with a double-run of it.
