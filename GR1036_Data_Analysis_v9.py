...
def auto_ingest_pipeline(mode="BOTH"):
    ...
    # Keep scanning until all expected slots are filled or 30 seconds elapses.
    ...
    while datetime.now().timestamp() < deadline:
        ...
        if mode == "LHS" or mode == "BOTH":
            ...
            for path, filename, _ in lhs_candidates:
                if len(g_lh_positions_db) == 5: break
                detected_pos = None
                for i in range(1, 6):
                    if f"pos{i}" in filename: detected_pos = i; break
                if detected_pos and detected_pos not in g_lh_positions_db:
                    try:
                        g_lh_positions_db[detected_pos] = load_data(path)
                        loaded_lh += 1
                    except Exception as e:
                        log_message(f"[INGEST] Skipped LHS '{os.path.basename(path)}': {e}")
        ...
        if mode == "RHS" or mode == "BOTH":
            ...
            for path, filename, _ in rhs_candidates:
                if len(g_rh_positions_db) == 5: break
                detected_pos = None
                for i in range(1, 6):
                    if f"pos{i}" in filename: detected_pos = i; break
                if detected_pos and detected_pos not in g_rh_positions_db:
                    try:
                        g_rh_positions_db[detected_pos] = load_data(path)
                        loaded_rh += 1
                    except Exception as e:
                        log_message(f"[INGEST] Skipped RHS '{os.path.basename(path)}': {e}")

        if loaded_lh >= expected_lh and loaded_rh >= expected_rh:
            break  # all expected files found and valid - no need to keep waiting

        time.sleep(0.5)  # wait 1s before retrying
...
def show_drift_chart(variant, position_idx):
    ...
    x_prim  = df['x_prim'].values  # Write the cleaned column data to variables for the calculation
    ...
    sc = ax.scatter(x_ghost * g_MM_PER_PX, y_ghost * g_MM_PER_PX, c=mag, cmap='viridis', s=marker_size,
                    edgecolor='black', linewidth=0.9, alpha=0.93, zorder=10,
                    label='Ghost points (with magnitude)')
    ax.quiver(x_prim * g_MM_PER_PX, y_prim * g_MM_PER_PX, u_dir, v_dir,
...
def thread_vb():
    ...
    # Initialize all per-loop-pass state before the while so values don't bleed between passes
            ...
            while g_system_running and session_active:
                try:
                    ...
                    if g_plc_rx_capture_results is True and not g_capture_results_armed:
                        ...
                        # Recipe selection: 0 = BOTH, 1 = LHS, 2 = RHS (per PLC spec)
...
tk.Label(global_frame, text="", font=("Arial", 7)).grid(row=1, column=0, pady=8)  # spacer for chart row label
for i in range(1, 6):
...
overall_status_lbl = tk.Label(global_frame, text="SYSTEM IDLE", bg="lightgray", fg="black",
                              font=("Arial", 25, "bold"), width=12, borderwidth=2, relief="solid")
