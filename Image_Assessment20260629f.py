...
def thread_vb():
    ...
    Architecture note: VB continuously sends messages even while idle (an "all false" filler/clear structure
    sent whenever it's waiting for a trigger), independent of anything Python sends. A strict 1-send-1-receive
    model can't keep up with that - any idle period lets VB's chatter pile up unread in the socket buffer,
    permanently desyncing every future read by one or more messages. So sending and receiving are split into
    two independent loops sharing the same connection: a continuous receiver that drains the socket constantly
    regardless of what's been sent, and a sender driven purely by PLC trigger gating with no receive tied to it.
    """
    #Python to PLC
    ...
    # Tracks the actual previous PLC RX trigger state, independent of the gate. Needed so a falling
    # edge is always detected and reported to VB, even if the prior round-trip never armed the gate
    # (e.g. VB replied with both TriggerComplete and TriggerFail false) - otherwise VB never finds out
    # the trigger went off, and the outbound mirror display gets stuck on its last sent value.
    l_prev_trigger_camera = False
    l_prev_capture_barcode = False
    ...
    while g_system_running:
        ...
        if g_connection_vb is None:
            try:
                ...
                # VB sends continuously while connected (real results plus idle filler messages), so the
                # receiver should basically never have to wait long. A short timeout here still catches a
                # genuinely dead connection (e.g. cable pulled, no clean close) reasonably promptly, without
                # being so long that a real failure takes ages to notice, and without being so short that
                # it could fire during a brief lull - 5s gives real headroom either way.
                l_connection_vb.settimeout(5.0)
                ...
                def vb_continuous_receiver(sock):
                    nonlocal session_active
                    global g_vb_rx_camera_ready, g_vb_rx_trigger_complete, g_vb_rx_trigger_fail
                    global g_vb_rx_barcode_complete, g_vb_rx_barcode_fail, g_vb_rx_position_echo
                    global g_vb_rx_scanned_barcode, g_plc_tx_barcode_string
                    global g_vb_camera_gate_pass, g_vb_camera_gate_fail
                    global g_vb_barcode_gate_pass, g_vb_barcode_gate_fail
                    global g_plc_tx_camera_pass, g_plc_tx_camera_fail
                    global g_plc_tx_barcode_pass, g_plc_tx_barcode_fail

                    while g_system_running and session_active:
                        try:
                            inbound_raw = sock.recv(54)  # blocks until VB sends something - which it does continuously
                            if not inbound_raw or len(inbound_raw) < 54:
                                raise socket.error("Connection closed by Vision Builder remote endpoint.")

                            rx_byte0, rx_byte1, rx_pos_echo, rx_scanned_barcode = struct.unpack("!BBH50s", inbound_raw[:54])
                            ...
                            # Only treat this as a genuine result (and update the gate) if VB actually reported
                            # something. VB's continuous idle/clear filler arrives as all-false and must be
                            # drained (so the socket never backs up) without being mistaken for a real result.
                            # Disambiguating camera vs barcode from the bits themselves (not from what was
                            # just sent) means this works correctly no matter how the send and receive sides
                            # happen to be timed relative to each other.
                            if g_vb_rx_trigger_complete or g_vb_rx_trigger_fail:
                            ...
                            if g_vb_rx_barcode_complete or g_vb_rx_barcode_fail:
                ...
                threading.Thread(target=vb_continuous_receiver, args=(l_connection_vb,), daemon=True).start()
            except Exception:
                with vbai_lock:
                    g_connection_vb = None
        ...
        #============================= Send ============================================================
        # Gate on the private gate fields rather than an internal latch: a trigger is sent only while
        # it's high AND we haven't already recorded a pass or fail for it. Those fields get set by the
        # continuous receiver above whenever VB reports a genuine result, and cleared below once the PLC
        # drops its trigger bit.
        #
        # Falling edge: detected from genuine previous-vs-current state (not gate state) so VB always
        # gets told the trigger went off, even if the prior round-trip's reply was ambiguous and never
        # armed the gate in the first place.
        try:
            ...
            camera_falling_edge = (l_prev_trigger_camera is True) and (g_plc_rx_trigger_camera is False)
            barcode_falling_edge = (l_prev_capture_barcode is True) and (g_plc_rx_capture_barcode is False)
            ...
            l_prev_trigger_camera = g_plc_rx_trigger_camera
            l_prev_capture_barcode = g_plc_rx_capture_barcode

            time.sleep(0.02)  # paces how often the PLC flags are re-checked while idle
